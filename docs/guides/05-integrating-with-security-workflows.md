---
title: "Integrating with Security Workflows"
sidebar_label: "Security Workflows"
sidebar_position: 5
description: "How to use Stave alongside other security tools and incident response processes."
---
...
stave apply --invariants ./invariants --observations ./observations --max-unsafe 0s --quiet

if [ $? -eq 3 ]; then
  echo "Deploy blocked: safety violations detected"
  stave apply --invariants ./invariants --observations ./observations --format text
  exit 1
fi
```
...
if [ "$violations" -gt 0 ]; then
  ...
  jq -r '.findings[] | "  \(.invariant_id): \(.resource_id)"' ./results/evaluation.json
...
## Incident Response: Breach-Type Scoping

For breach-type-specific evaluation, use the `--context` flag to focus on invariants relevant to the incident type:
...
With `breach_type: DISC` (disclosure), Stave evaluates only disclosure-relevant invariants:
...
### Service Control Policy (AWS SCP)
