...
**ID:** `CTL.S3.LOG.001` **Category:** Logging **Severity:** Medium
...
* [`CTL.S3.GOVERNANCE.001`](/docs/reference/invariants-reference/lifecycle-governance/inv-s3-governance-001.md) -- Data Classification Tag Required. Buckets without a `data-classification` tag silently bypass sensitivity-gated checks. Logging and classification together ensure that access to sensitive buckets is both tracked and categorizable.
