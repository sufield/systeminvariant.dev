...
### CTL.EKS.ACCESSENTRY.CLUSTERADMIN.BROAD.001[​](#ctleksaccessentryclusteradminbroad001 "Direct link to CTL.EKS.ACCESSENTRY.CLUSTERADMIN.BROAD.001")
...
* **Type:** unsafe\_state
* **Domain:** identity
* **Compliance:** fedramp\_moderate: AC-3, AC-6; iso\_27001\_2022: A.5.15, A.8.2; nist\_800\_53\_r5: AC-3, AC-6; pci\_dss\_v4.0: 7.2; soc2: CC6.1, CC6.3;
...
### CTL.EKS.ACCESSENTRY.GHOST.PRINCIPAL.001[​](#ctleksaccessentryghostprincipal001 "Direct link to CTL.EKS.ACCESSENTRY.GHOST.PRINCIPAL.001")
...
* **Type:** unsafe\_state
* **Domain:** identity
* **Compliance:** fedramp\_moderate: AC-3, CM-2; iso\_27001\_2022: A.5.15, A.5.16, A.8.32; nist\_800\_53\_r5: AC-3, CM-2, CM-3; soc2: CC6.1, CC8.1, CC7.4;
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: CM-2, IA-5; iso\_27001\_2022: A.5.16, A.8.32; nist\_800\_53\_r5: CM-2, CM-3, IA-5; soc2: CC6.1, CC8.1, CC7.4;
...
**Remediation:** Recreate the IAM role with the documented IRSA trust policy for the add-on (each add-on's docs publish the required policy), or update the add-on configuration via aws eks update-addon --service-account-role-arn . After fixing, restart the add-on's pods to pick up the new role.

***

### CTL.EKS.ADDON.HELMDUAL.001[​](#ctleksaddonhelmdual001 "Direct link to CTL.EKS.ADDON.HELMDUAL.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: CM-2; iso\_27001\_2022: A.5.16, A.8.32; nist\_800\_53\_r5: CM-2, CM-3; soc2: CC8.1;
...
### CTL.EKS.ADDON.SELFMANAGED.001[​](#ctleksaddonselfmanaged001 "Direct link to CTL.EKS.ADDON.SELFMANAGED.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: CM-2, SI-2; iso\_27001\_2022: A.5.16, A.8.8; nist\_800\_53\_r5: CM-2, SI-2; soc2: CC8.1, A1.2;
...
**Remediation:** Migrate to EKS managed add-ons: aws eks create-addon --cluster-name --addon-name vpc-cni --addon-version Use the `--resolve-conflicts OVERWRITE` flag carefully if the cluster already has the self-managed install. After migration, the self-managed Helm release / manifests can be deleted; EKS managed add-ons now own the component.

***

### CTL.EKS.ADDON.VERSION.001[​](#ctleksaddonversion001 "Direct link to CTL.EKS.ADDON.VERSION.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** aws\_security\_hub: EKS.7; mitre\_attack: TA0003; nist\_800\_53\_r5: SI-2;
...
**Remediation:** List available updates: aws eks describe-addon-versions --kubernetes-version . Update each addon: aws eks update-addon --cluster-name --addon-name --addon-version --resolve-conflicts OVERWRITE

***

### CTL.EKS.ALARM.API.ERRORS.001[​](#ctleksalarmapierrors001 "Direct link to CTL.EKS.ALARM.API.ERRORS.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: SI-4, AU-6; iso\_27001\_2022: A.8.16; nist\_800\_53\_r5: SI-4, AU-6; soc2: CC7.2, A1.1;

EKS cluster has no CloudWatch alarm on the apiserver\_request\_total metric scoped to 5xx (server errors) or persistent 4xx (client errors). The API server is the single coordination point for the cluster; degradation there cascades to every controller, scheduler, and operator. Without an alarm, error-rate spikes go unnoticed until downstream symptoms surface (deploys hanging, scaling failures).

**Remediation:** Enable EKS control-plane logs (api type) into CloudWatch Logs, then create a metric filter on apiserver\_request\_total with code=\~"5..". Wire a CloudWatch alarm to SNS for sustained error rate above a threshold (e.g., >1% over 5 minutes). Container Insights enhanced observability also exposes apiserver metrics directly.

***

### CTL.EKS.ALARM.CRASHLOOP.001[​](#ctleksalarmcrashloop001 "Direct link to CTL.EKS.ALARM.CRASHLOOP.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: SI-4; iso\_27001\_2022: A.8.16; nist\_800\_53\_r5: SI-4; soc2: CC7.2, A1.1;
...
**Remediation:** Enable Container Insights. Create CloudWatch alarm on pod\_status\_phase{status="CrashLoopBackOff"} over 5 minutes. Per-namespace dimensions let teams subscribe to alarms scoped to their own workloads.

***

### CTL.EKS.ALARM.ETCD.LATENCY.001[​](#ctleksalarmetcdlatency001 "Direct link to CTL.EKS.ALARM.ETCD.LATENCY.001")
...
EKS cluster has no CloudWatch alarm on etcd\_request\_duration\_seconds p99 latency. etcd serves every API-server read and write; latency there directly determines kubectl + controller responsiveness. AWS-managed control plane handles etcd availability, but tenant-driven cluster shape (large numbers of CRDs, many Secrets, frequent audit-log writes) drives latency that AWS doesn't alert on for the tenant. EKS Container Insights enhanced observability exposes etcd metrics directly.

**Remediation:** Enable Container Insights enhanced observability for the cluster. Create a CloudWatch alarm on etcd\_request\_duration\_seconds p99 above a baseline threshold (typical: >500ms over 5 minutes for healthy clusters). For high-Secret-density clusters, watch the etcd\_db\_total\_size\_in\_bytes metric — approaching 8GiB triggers etcd compaction issues.

***

### CTL.EKS.ALARM.KUBELET.HEARTBEAT.001[​](#ctleksalarmkubeletheartbeat001 "Direct link to CTL.EKS.ALARM.KUBELET.HEARTBEAT.001")
...
**Remediation:** Enable Container Insights enhanced observability plus the kubelet\_node\_lease\_renewal\_failure\_total metric scrape. Create a CloudWatch alarm on sustained heartbeat-loss rate per node. Pair with the NodeNotReady alarm (which is downstream of heartbeat loss) for layered detection.

***

### CTL.EKS.ALARM.NODE.PRESSURE.001[​](#ctleksalarmnodepressure001 "Direct link to CTL.EKS.ALARM.NODE.PRESSURE.001")
...
**Remediation:** Enable Container Insights metrics. Create a CloudWatch alarm on cluster\_node\_count - cluster\_node\_running\_count (the diff is NotReady nodes). Threshold of >1 over 5 minutes catches most node failures while avoiding spurious alarms from rolling node replacements.

***

### CTL.EKS.ALARM.OOMKILLED.001[​](#ctleksalarmoomkilled001 "Direct link to CTL.EKS.ALARM.OOMKILLED.001")
...
**Remediation:** Enable Container Insights. Create CloudWatch alarm on container\_termination{reason=OOMKilled} sum over 5 minutes. Dimension by namespace + pod for team-specific routing. Workloads persistently OOMKilling indicate either memory leak (apply heap profiling) or undersized limits (review actual usage and adjust).

***

### CTL.EKS.ALARM.PODEVICTED.001[​](#ctleksalarmpodevicted001 "Direct link to CTL.EKS.ALARM.PODEVICTED.001")
...
**Remediation:** Enable Container Insights. Create CloudWatch alarm on pod\_status\_phase{status="Failed",reason="Evicted"} over 5 minutes. Persistent evictions point at either capacity issues (scale up nodes / add requests-correctness) or PriorityClass contention (review per-namespace PriorityClass usage).

***

### CTL.EKS.ARGOCD.SPOKE.ADMIN.001[​](#ctleksargocdspokeadmin001 "Direct link to CTL.EKS.ARGOCD.SPOKE.ADMIN.001")
...
### CTL.EKS.AUDIT.RETENTION.SHORT.001[​](#ctleksauditretentionshort001 "Direct link to CTL.EKS.AUDIT.RETENTION.SHORT.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: AU-11; hipaa: 164.530(j); iso\_27001\_2022: A.5.33; nist\_800\_53\_r5: AU-11; pci\_dss\_v4.0: 10.5; soc2: CC7.2;
...
**Remediation:** Update the CloudWatch Logs group's retention to match the cluster's compliance scope: aws logs put-retention-policy --log-group-name /aws/eks//cluster --retention-in-days 2557 For HIPAA: 2557 days (7 years to be safe). For PCI: 365. For SOC2: 365 minimum. Pair with log archival to S3 Glacier for cost-effective long-retention.

***

### CTL.EKS.AUTH.MIXED.001[​](#ctleksauthmixed001 "Direct link to CTL.EKS.AUTH.MIXED.001")
...
* **Type:** unsafe\_state
* **Domain:** identity
* **Compliance:** fedramp\_moderate: AC-3, CM-2; iso\_27001\_2022: A.5.15, A.8.32; nist\_800\_53\_r5: AC-3, CM-2; soc2: CC6.1, CC8.1;

EKS cluster's authentication mode is set to API\_AND\_CONFIG\_MAP (or has both legacy aws-auth ConfigMap mappings and modern EKS Access Entries) without explicit migration plan. Two sources of truth for cluster authentication is fragile: changes to either surface can change effective access without the operator realizing both must be reviewed; ghost entries can accumulate in either independently; and audits scanning only one surface miss grants from the other. The cluster should commit to one authentication mode and finish the migration.

**Remediation:** Migrate to Access Entries (recommended) or revert to aws-auth-only (rare). For Access Entries migration: (1) audit aws-auth, (2) recreate every mapping as an Access Entry, (3) set authentication\_mode to API, (4) clear the aws-auth ConfigMap. Don't switch authentication\_mode to API while aws-auth still has mappings — workloads depending on those mappings will lose access.

***

### CTL.EKS.AWSAUTH.GHOST.ROLE.001[​](#ctleksawsauthghostrole001 "Direct link to CTL.EKS.AWSAUTH.GHOST.ROLE.001")
...
* **Type:** unsafe\_state
* **Domain:** identity
* **Compliance:** fedramp\_moderate: AC-3, CM-2; iso\_27001\_2022: A.5.15, A.5.16, A.8.32; nist\_800\_53\_r5: AC-3, CM-2, CM-3; soc2: CC6.1, CC6.3;
...
### CTL.EKS.AWSAUTH.MASTERS.BROAD.001[​](#ctleksawsauthmastersbroad001 "Direct link to CTL.EKS.AWSAUTH.MASTERS.BROAD.001")
...
### CTL.EKS.AWSAUTH.UNTRUSTED.ACCOUNT.001[​](#ctleksawsauthuntrustedaccount001 "Direct link to CTL.EKS.AWSAUTH.UNTRUSTED.ACCOUNT.001")
...
* **Type:** unsafe\_state
* **Domain:** identity
* **Compliance:** fedramp\_moderate: AC-3, AC-6; iso\_27001\_2022: A.5.15, A.8.2; nist\_800\_53\_r5: AC-3, AC-4, AC-6; pci\_dss\_v4.0: 7.2; soc2: CC6.1, CC6.3, CC6.6;
...
### CTL.EKS.CLUSTER.VERSION.001[​](#ctleksclusterversion001 "Direct link to CTL.EKS.CLUSTER.VERSION.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** aws\_security\_hub: EKS.6; mitre\_attack: TA0003; nist\_800\_53\_r5: SI-2;
...
**Remediation:** Check current version: aws eks describe-cluster --name --query 'cluster.version'. Upgrade: aws eks update-cluster-version --name --kubernetes-version . Then upgrade node groups.

***

### CTL.EKS.CONTROL.PLANE.AUDIT.001[​](#ctlekscontrolplaneaudit001 "Direct link to CTL.EKS.CONTROL.PLANE.AUDIT.001")
...
* **Type:** unsafe\_state
* **Domain:** audit
* **Compliance:** cis\_eks: 2.1; mitre\_attack: TA0005; nist\_800\_53\_r5: AU-2;
...
**Remediation:** Enable audit logging: aws eks update-cluster-config --name --logging '{"clusterLogging":\[{"types":\["audit"],"enabled":true}]}'. Verify the log group exists in CloudWatch.

***

### CTL.EKS.COREDNS.AUTOSCALE.001[​](#ctlekscorednsautoscale001 "Direct link to CTL.EKS.COREDNS.AUTOSCALE.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: SC-5; iso\_27001\_2022: A.8.6; nist\_800\_53\_r5: SC-5; soc2: A1.1, CC7.2;
...
**Remediation:** Deploy cluster-proportional-autoscaler for CoreDNS: kubectl apply -f <https://github.com/kubernetes-sigs/> cluster-proportional-autoscaler/... Configure scaling parameters (`coresPerReplica: 256, nodesPerReplica: 16`). Alternative: HPA on CoreDNS deployment with CPU target. AWS-recommended for production EKS clusters above 50 nodes.

***

### CTL.EKS.CSI.EBS.NOIRSA.001[​](#ctlekscsiebsnoirsa001 "Direct link to CTL.EKS.CSI.EBS.NOIRSA.001")
...
**Remediation:** Create an IAM role with the AmazonEBSCSIDriverPolicy (AWS-managed minimal policy). Trust policy must include the cluster's OIDC issuer + sub for the ebs-csi-controller-sa ServiceAccount. Annotate the SA: kubectl annotate sa ebs-csi-controller-sa -n kube-system eks.amazonaws.com/role-arn= Restart the controller deployment to pick up IRSA.

***

### CTL.EKS.CSI.EFS.NOIRSA.001[​](#ctlekscsiefsnoirsa001 "Direct link to CTL.EKS.CSI.EFS.NOIRSA.001")
...
### CTL.EKS.DELETEPROT.001[​](#ctleksdeleteprot001 "Direct link to CTL.EKS.DELETEPROT.001")
...
**Remediation:** Enable deletion protection via aws eks update-cluster-config --name --deletion-protection.

***

### CTL.EKS.DR.KMS.SINGLEREGION.001[​](#ctleksdrkmssingleregion001 "Direct link to CTL.EKS.DR.KMS.SINGLEREGION.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: CP-9, SC-12; iso\_27001\_2022: A.5.30, A.8.24; nist\_800\_53\_r5: CP-9, SC-12, SC-28; soc2: A1.2, CC6.1;
...
### CTL.EKS.DR.WARMUP.001[​](#ctleksdrwarmup001 "Direct link to CTL.EKS.DR.WARMUP.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: CP-7, CP-9, CP-10; iso\_27001\_2022: A.5.30, A.8.13; nist\_800\_53\_r5: CP-7, CP-9, CP-10; soc2: A1.1, A1.2, CC7.4;
...
### CTL.EKS.EBS.ORPHAN.001[​](#ctleksebsorphan001 "Direct link to CTL.EKS.EBS.ORPHAN.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: CM-2; iso\_27001\_2022: A.5.16, A.8.10; nist\_800\_53\_r5: CM-2, CM-8; soc2: CC8.1;
...
**Remediation:** Snapshot the volumes (preserves data if needed), then delete via aws ec2 delete-volume. Audit via: aws ec2 describe-volumes --filters Name=status,Values=available Name=tag-key,Values=kubernetes.io/cluster Set up a recurring CloudWatch Events / Lambda to alert on / auto-delete EKS-tagged orphan volumes after a documented retention period.

***

### CTL.EKS.ECR.PULL.PUBLIC.GALLERY.001[​](#ctleksecrpullpublicgallery001 "Direct link to CTL.EKS.ECR.PULL.PUBLIC.GALLERY.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: CM-2, SR-3; iso\_27001\_2022: A.5.16, A.8.30; nist\_800\_53\_r5: CM-2, SR-3; soc2: CC6.1, CC8.1;
...
### CTL.EKS.ECR.REPLICATION.GAP.001[​](#ctleksecrreplicationgap001 "Direct link to CTL.EKS.ECR.REPLICATION.GAP.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: CP-9, CM-2; iso\_27001\_2022: A.5.30, A.8.13; nist\_800\_53\_r5: CP-9, CM-2; soc2: A1.2, CC8.1;
...
### CTL.EKS.ECR.SCAN.EXPORT.001[​](#ctleksecrscanexport001 "Direct link to CTL.EKS.ECR.SCAN.EXPORT.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: RA-5, SI-4; iso\_27001\_2022: A.5.16, A.8.8, A.8.16; nist\_800\_53\_r5: RA-5, SI-4; soc2: CC7.2, CC8.1;
...
**Remediation:** Enable Security Hub in the region. Enable the Inspector integration with Security Hub (Inspector is the native AWS service that scans ECR images and forwards findings). Verify findings appear: aws securityhub get-findings --filters ProductName=Inspector Create a Security Hub insight on Severity=CRITICAL+HIGH for routing to the on-call queue.

***

### CTL.EKS.ENDPOINT.PRIVATE.001[​](#ctleksendpointprivate001 "Direct link to CTL.EKS.ENDPOINT.PRIVATE.001")
...
**Remediation:** Enable private endpoint access via aws eks update-cluster-config --name --resources-vpc-config endpointPrivateAccess=true.

***

### CTL.EKS.FARGATE.DAEMONSET.001[​](#ctleksfargatedaemonset001 "Direct link to CTL.EKS.FARGATE.DAEMONSET.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: CM-2, SI-4; iso\_27001\_2022: A.5.16, A.8.16; nist\_800\_53\_r5: CM-2, SI-4; soc2: CC7.2, A1.1;
...
### CTL.EKS.FARGATE.PROFILE.OVERPRIV.001[​](#ctleksfargateprofileoverpriv001 "Direct link to CTL.EKS.FARGATE.PROFILE.OVERPRIV.001")
...
### CTL.EKS.FARGATE.SELECTOR.WIDE.001[​](#ctleksfargateselectorwide001 "Direct link to CTL.EKS.FARGATE.SELECTOR.WIDE.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: CM-2; iso\_27001\_2022: A.5.16, A.8.20; nist\_800\_53\_r5: CM-2, AC-3; soc2: CC8.1, A1.1;
...
### CTL.EKS.GUARDDUTY.EKS.PROTECTION.001[​](#ctleksguarddutyeksprotection001 "Direct link to CTL.EKS.GUARDDUTY.EKS.PROTECTION.001")
...
* **Type:** unsafe\_state
* **Domain:** detection
* **Compliance:** fedramp\_moderate: SI-4, AU-6; iso\_27001\_2022: A.5.24, A.8.16; nist\_800\_53\_r5: SI-4, IR-4, AU-6; pci\_dss\_v4.0: 10.2, 11.5; soc2: CC7.2, CC7.3;
...
**Remediation:** Enable GuardDuty in the region; enable EKS Protection (audit log monitoring) under GuardDuty's Kubernetes protection settings: aws guardduty update-detector --detector-id --features Name=EKS\_AUDIT\_LOGS,Status=ENABLED Verify findings flow to Security Hub for central operations dashboard.

***

### CTL.EKS.GUARDDUTY.RUNTIME.001[​](#ctleksguarddutyruntime001 "Direct link to CTL.EKS.GUARDDUTY.RUNTIME.001")
...
* **Type:** unsafe\_state
* **Domain:** detection
* **Compliance:** fedramp\_moderate: SI-4; iso\_27001\_2022: A.5.24, A.8.16; nist\_800\_53\_r5: SI-4, IR-4; pci\_dss\_v4.0: 11.5; soc2: CC7.2, CC7.3;
...
**Remediation:** Enable Runtime Monitoring in GuardDuty for the region. Configure automated agent management: aws guardduty update-detector --detector-id --features 'Name=RUNTIME\_MONITORING,Status=ENABLED, AdditionalConfiguration=\[{Name=EKS\_ADDON\_MANAGEMENT, Status=ENABLED}]' The aws-guardduty-agent EKS add-on deploys automatically to the cluster. Findings flow to Security Hub.

***

### CTL.EKS.HELM.RELEASE.FAILED.001[​](#ctlekshelmreleasefailed001 "Direct link to CTL.EKS.HELM.RELEASE.FAILED.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: CM-2; iso\_27001\_2022: A.5.16, A.8.32; nist\_800\_53\_r5: CM-2, CM-3; soc2: CC8.1, CC7.4;
...
**Remediation:** Decide whether to retry or remove. Retry: helm rollback 0 (or upgrade with --reset-values to a known-good chart version). Remove: helm uninstall deletes the release record + cluster resources. Audit cluster Helm state regularly: helm list -A --pending --failed

***

### CTL.EKS.HPA.GHOST.TARGET.001[​](#ctlekshpaghosttarget001 "Direct link to CTL.EKS.HPA.GHOST.TARGET.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: CM-2; iso\_27001\_2022: A.5.16, A.8.32; nist\_800\_53\_r5: CM-2, CM-3; soc2: CC8.1, A1.1;
...
**Remediation:** Delete the orphan HPA via kubectl delete hpa -n , OR repoint the scaleTargetRef to a live workload if the target was renamed. Audit cluster HPAs for orphan targets via kubectl get hpa -A and verify every scaleTargetRef resolves.

***

### CTL.EKS.IMAGE.BASE.STALE.001[​](#ctleksimagebasestale001 "Direct link to CTL.EKS.IMAGE.BASE.STALE.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: SI-2, RA-5; iso\_27001\_2022: A.5.16, A.8.8; nist\_800\_53\_r5: SI-2, RA-5; pci\_dss\_v4.0: 6.3, 11.2; soc2: CC8.1, CC7.2;
...
### CTL.EKS.IMAGE.CVE.BLOCK.001[​](#ctleksimagecveblock001 "Direct link to CTL.EKS.IMAGE.CVE.BLOCK.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: RA-5, SI-2; iso\_27001\_2022: A.5.16, A.8.8; nist\_800\_53\_r5: RA-5, SI-2; pci\_dss\_v4.0: 11.2; soc2: CC8.1, CC7.2;
...
### CTL.EKS.IMAGE.LATEST.TAG.001[​](#ctleksimagelatesttag001 "Direct link to CTL.EKS.IMAGE.LATEST.TAG.001")
...
**Remediation:** Replace :latest with a specific version tag (semver, Git SHA, or build ID). For maximum reproducibility, pin by SHA256 digest: image: 111122223333.dkr.ecr.us-east-1.amazonaws.com/ orders\@sha256:abc123... Tags are convenient pointers; digests are cryptographic identifiers. Production should pin by digest where possible.

***

### CTL.EKS.IMAGE.NODIGEST.001[​](#ctleksimagenodigest001 "Direct link to CTL.EKS.IMAGE.NODIGEST.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: CM-2; iso\_27001\_2022: A.5.16, A.8.30; nist\_800\_53\_r5: CM-2, SR-3; soc2: CC8.1, CC7.4;
...
### CTL.EKS.IMAGE.NOSIGNATURE.001[​](#ctleksimagenosignature001 "Direct link to CTL.EKS.IMAGE.NOSIGNATURE.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: CM-2, SR-3; iso\_27001\_2022: A.5.16, A.8.30; nist\_800\_53\_r5: CM-2, SR-3, SR-4; pci\_dss\_v4.0: 6.3; soc2: CC6.1, CC8.1;
...
**Remediation:** Install Kyverno (or equivalent) and apply a verifyImages policy: apiVersion: kyverno.io/v1 kind: ClusterPolicy metadata: { name: verify-image-signatures } spec: validationFailureAction: Enforce rules: - name: verify-signed match: { any: \[{ resources: { kinds: \[Pod] } }] } verifyImages: - imageReferences: \["111122223333.dkr.ecr.\*"] attestors: \[...] # public key or Cosign keyless Sign images during CI build with cosign sign or AWS Signer; the policy rejects unsigned pods.

***

### CTL.EKS.IMAGE.PUBLIC.REGISTRY.001[​](#ctleksimagepublicregistry001 "Direct link to CTL.EKS.IMAGE.PUBLIC.REGISTRY.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: CM-2, SI-3; iso\_27001\_2022: A.5.16, A.8.30; nist\_800\_53\_r5: CM-2, SI-3, SR-3; pci\_dss\_v4.0: 6.3; soc2: CC6.1, CC8.1;
...
### CTL.EKS.IMAGE.PULLPOLICY.NEVER.001[​](#ctleksimagepullpolicynever001 "Direct link to CTL.EKS.IMAGE.PULLPOLICY.NEVER.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: CM-2; iso\_27001\_2022: A.5.16, A.8.32; nist\_800\_53\_r5: CM-2; soc2: CC8.1, CC7.4;
...
### CTL.EKS.IMAGE.PULLSECRET.SHARED.001[​](#ctleksimagepullsecretshared001 "Direct link to CTL.EKS.IMAGE.PULLSECRET.SHARED.001")
...
* **Type:** unsafe\_state
* **Domain:** identity
* **Compliance:** fedramp\_moderate: AC-3, IA-5; iso\_27001\_2022: A.5.17, A.8.5; nist\_800\_53\_r5: AC-3, IA-5; pci\_dss\_v4.0: 8.3; soc2: CC6.1, CC6.3;
...
### CTL.EKS.INGRESS.GHOST.CERT.001[​](#ctleksingressghostcert001 "Direct link to CTL.EKS.INGRESS.GHOST.CERT.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: CM-2, SC-13; iso\_27001\_2022: A.5.16, A.8.24, A.8.32; nist\_800\_53\_r5: CM-2, CM-3, SC-13; soc2: CC6.1, CC6.7, CC8.1;
...
**Remediation:** Update the Ingress annotation to a live ACM certificate ARN (kubectl edit ingress ), or recreate the deleted certificate at the original ARN if it was intentional and the original is still needed. Pair with cert expiry monitoring so future ACM deletions / expiry surfaces immediately.

***

### CTL.EKS.IRSA.AUDIENCE.MISSING.001[​](#ctleksirsaaudiencemissing001 "Direct link to CTL.EKS.IRSA.AUDIENCE.MISSING.001")
...
* **Type:** unsafe\_state
* **Domain:** identity
* **Compliance:** fedramp\_moderate: AC-3, IA-5; iso\_27001\_2022: A.5.15, A.8.2; nist\_800\_53\_r5: AC-3, IA-5; pci\_dss\_v4.0: 7.2; soc2: CC6.1, CC6.3;
...
**Remediation:** Add `<oidc-issuer>:aud` = `sts.amazonaws.com` to the trust policy's StringEquals condition. The full per-workload pattern is: "Condition": { "StringEquals": { ":sub": "system:serviceaccount::", ":aud": "sts.amazonaws.com" } }

***

### CTL.EKS.IRSA.CROSSACCOUNT.001[​](#ctleksirsacrossaccount001 "Direct link to CTL.EKS.IRSA.CROSSACCOUNT.001")
...
* **Type:** unsafe\_state
* **Domain:** identity
* **Compliance:** fedramp\_moderate: AC-3, IA-5; iso\_27001\_2022: A.5.15, A.8.2; nist\_800\_53\_r5: AC-3, AC-4, IA-5; pci\_dss\_v4.0: 7.2; soc2: CC6.1, CC6.3, CC6.6;
...
### CTL.EKS.IRSA.ENFORCE.001[​](#ctleksirsaenforce001 "Direct link to CTL.EKS.IRSA.ENFORCE.001")
...
* **Type:** unsafe\_state
* **Domain:** identity
* **Compliance:** aws\_security\_hub: EKS.9; mitre\_attack: TA0006; nist\_800\_53\_r5: AC-6;
...
**Remediation:** Associate an OIDC provider: eksctl utils associate-iam-oidc-provider --cluster --approve. Then create service account-specific IAM roles.

***

### CTL.EKS.IRSA.OIDC.GHOST.001[​](#ctleksirsaoidcghost001 "Direct link to CTL.EKS.IRSA.OIDC.GHOST.001")
...
**Remediation:** Verify the OIDC provider exists: aws iam list-open-id-connect-providers. Confirm the URL in the trust policy matches exactly — including the region, the cluster ID segment, and no trailing slash. If the cluster OIDC provider was deleted, recreate via eksctl utils associate-iam-oidc-provider --cluster . Update the trust policy with the new ARN if it changed.

***

### CTL.EKS.IRSA.SUB.UNSCOPED.001[​](#ctleksirsasubunscoped001 "Direct link to CTL.EKS.IRSA.SUB.UNSCOPED.001")
...
* **Type:** unsafe\_state
* **Domain:** identity
* **Compliance:** fedramp\_moderate: AC-3, AC-6; iso\_27001\_2022: A.5.15, A.8.2; nist\_800\_53\_r5: AC-3, AC-6, IA-5; pci\_dss\_v4.0: 7.2; soc2: CC6.1, CC6.3;
...
**Remediation:** Add a `sub` condition to the trust policy: "Condition": { "StringEquals": { ":sub": "system:serviceaccount::", ":aud": "sts.amazonaws.com" } } Pin to the exact namespace + SA name. For roles serving multiple workloads, list each `sub` explicitly via StringEquals on an array.

***

### CTL.EKS.LOGGING.001[​](#ctlekslogging001 "Direct link to CTL.EKS.LOGGING.001")
...
* **Type:** unsafe\_state
* **Domain:** exposure
* **Compliance:** cis\_eks: 2.1; fedramp\_moderate: AU-12; hipaa: 164.312(b); nist\_800\_53\_r5: AU-2;
...
**Remediation:** Enable all five control plane log types via AWS CLI: aws eks update-cluster-config --name --logging '{"clusterLogging":\[{"types":\["api","audit","authenticator", "controllerManager","scheduler"],"enabled":true}]}'

***

### CTL.EKS.NETPOL.EGRESS.OPEN.001[​](#ctleksnetpolegressopen001 "Direct link to CTL.EKS.NETPOL.EGRESS.OPEN.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: AC-4, SC-7; iso\_27001\_2022: A.5.15, A.8.20; nist\_800\_53\_r5: AC-4, SC-7; pci\_dss\_v4.0: 1.2, 1.3; soc2: CC6.1, CC6.6;
...
### CTL.EKS.NETPOL.ENFORCE.001[​](#ctleksnetpolenforce001 "Direct link to CTL.EKS.NETPOL.ENFORCE.001")
...
**Remediation:** Enable network policy enforcement via VPC CNI: aws eks update-addon --cluster-name --addon-name vpc-cni --configuration-values '{"enableNetworkPolicy": "true"}'. Then apply default-deny NetworkPolicy in each namespace.

***

### CTL.EKS.NETPOL.GHOST.PEER.001[​](#ctleksnetpolghostpeer001 "Direct link to CTL.EKS.NETPOL.GHOST.PEER.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: AC-3, CM-2; iso\_27001\_2022: A.5.16, A.8.16, A.8.32; nist\_800\_53\_r5: AC-3, CM-2, CM-3; soc2: CC6.1, CC8.1, CC7.4;
...
**Remediation:** Either delete the orphan policy (kubectl delete networkpolicy ) if the workload it protected is decommissioned, or update the selectors to match the current workload labels if the labels have shifted. Audit cluster network policies via `kubectl get netpol -A` + a label-match dry run.

***

### CTL.EKS.NETPOL.INGRESS.OPEN.001[​](#ctleksnetpolingressopen001 "Direct link to CTL.EKS.NETPOL.INGRESS.OPEN.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: AC-3, SC-7; iso\_27001\_2022: A.5.15, A.8.20; nist\_800\_53\_r5: AC-3, AC-4, SC-7; pci\_dss\_v4.0: 1.2, 1.3; soc2: CC6.1, CC6.6;
...
### CTL.EKS.NETPOL.INGRESSONLY.001[​](#ctleksnetpolingressonly001 "Direct link to CTL.EKS.NETPOL.INGRESSONLY.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: AC-4, SC-7; iso\_27001\_2022: A.5.15, A.8.20; nist\_800\_53\_r5: AC-4, SC-7; soc2: CC6.1, CC6.6;
...
### CTL.EKS.NETPOL.NODEFAULTDENY.EGRESS.001[​](#ctleksnetpolnodefaultdenyegress001 "Direct link to CTL.EKS.NETPOL.NODEFAULTDENY.EGRESS.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: AC-4, SC-7; iso\_27001\_2022: A.5.15, A.8.20; nist\_800\_53\_r5: AC-4, SC-7; pci\_dss\_v4.0: 1.2, 1.3; soc2: CC6.1, CC6.6, CC6.7;
...
**Remediation:** Apply a default-deny baseline: apiVersion: networking.k8s.io/v1 kind: NetworkPolicy metadata: name: default-deny-egress namespace: spec: podSelector: {} policyTypes: \[Egress] Then explicitly allow only the egress destinations the workload requires (internal cluster services, AWS endpoints via VPC endpoints, specific external APIs). Block IMDS access by including its IP range in deny rules.

***

### CTL.EKS.NETPOL.NODEFAULTDENY.INGRESS.001[​](#ctleksnetpolnodefaultdenyingress001 "Direct link to CTL.EKS.NETPOL.NODEFAULTDENY.INGRESS.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: AC-3, SC-7; iso\_27001\_2022: A.5.15, A.8.20; nist\_800\_53\_r5: AC-3, AC-4, SC-7; pci\_dss\_v4.0: 1.2, 1.3; soc2: CC6.1, CC6.6, CC6.7;
...
**Remediation:** Apply a default-deny baseline: apiVersion: networking.k8s.io/v1 kind: NetworkPolicy metadata: name: default-deny-ingress namespace: spec: podSelector: {} policyTypes: \[Ingress] Then layer explicit allow policies for the connections the workload actually needs. Default-deny + explicit allow is the documented Kubernetes pattern for namespace-level network isolation.

***

### CTL.EKS.NETPOL.NSSELECTOR.WIDE.001[​](#ctleksnetpolnsselectorwide001 "Direct link to CTL.EKS.NETPOL.NSSELECTOR.WIDE.001")
...
### CTL.EKS.NODEGROUP.AMI.001[​](#ctleksnodegroupami001 "Direct link to CTL.EKS.NODEGROUP.AMI.001")
...
### CTL.EKS.NODEGROUP.BOOTSTRAP.UNPINNED.001[​](#ctleksnodegroupbootstrapunpinned001 "Direct link to CTL.EKS.NODEGROUP.BOOTSTRAP.UNPINNED.001")
...
### CTL.EKS.NODEGROUP.IMDS.DISABLED.001[​](#ctleksnodegroupimdsdisabled001 "Direct link to CTL.EKS.NODEGROUP.IMDS.DISABLED.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: CM-2; iso\_27001\_2022: A.5.16, A.8.32; nist\_800\_53\_r5: CM-2, SI-4; soc2: CC8.1, CC7.4;
...
### CTL.EKS.NODEGROUP.IMDS.HOPLIMIT.001[​](#ctleksnodegroupimdshoplimit001 "Direct link to CTL.EKS.NODEGROUP.IMDS.HOPLIMIT.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: AC-3, IA-2; iso\_27001\_2022: A.5.15, A.8.16; nist\_800\_53\_r5: AC-3, IA-2, SC-7; pci\_dss\_v4.0: 7.2; soc2: CC6.1, CC6.3;
...
### CTL.EKS.NODEGROUP.IMDSV2.001[​](#ctleksnodegroupimdsv2001 "Direct link to CTL.EKS.NODEGROUP.IMDSV2.001")
...
**Remediation:** Update the launch template's MetadataOptions to require IMDSv2: MetadataOptions: HttpTokens: required HttpPutResponseHopLimit: 1 HttpEndpoint: enabled Roll the node group to apply (replace nodes). Verify every node group in the cluster — IMDSv2 must be cluster-wide; one node group permitting v1 is enough for an SSRF-targeted pod.

***

### CTL.EKS.NODEGROUP.PROFILE.SHARED.001[​](#ctleksnodegroupprofileshared001 "Direct link to CTL.EKS.NODEGROUP.PROFILE.SHARED.001")
...
### CTL.EKS.NODEGROUP.SELFMANAGED.NOTREADY.001[​](#ctleksnodegroupselfmanagednotready001 "Direct link to CTL.EKS.NODEGROUP.SELFMANAGED.NOTREADY.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: CM-2, SI-4; iso\_27001\_2022: A.5.16, A.8.16; nist\_800\_53\_r5: CM-2, CM-8, SI-4; soc2: CC8.1, A1.1;
...
* **Type:** unsafe\_state
* **Domain:** network
* **Compliance:** mitre\_attack: TA0008; nist\_800\_53\_r5: SC-7;
...
### CTL.EKS.NODEGROUP.SSH.OPEN.001[​](#ctleksnodegroupsshopen001 "Direct link to CTL.EKS.NODEGROUP.SSH.OPEN.001")
...
* **Type:** unsafe\_state
* **Domain:** exposure
* **Compliance:** fedramp\_moderate: AC-3, SC-7; iso\_27001\_2022: A.5.15, A.8.16; nist\_800\_53\_r5: AC-3, AC-17, SC-7; pci\_dss\_v4.0: 1.2, 2.2; soc2: CC6.1, CC6.6;
...
**Remediation:** Remove the SSH inbound rule from the worker SG. For operational access, deploy SSM Agent on the AMI (the EKS-optimized AMI includes it) and use Session Manager: aws ssm start-session --target Session Manager doesn't require open inbound ports, integrates with IAM for authorization, and logs every session to CloudWatch Logs for audit.

***

### CTL.EKS.NODEGROUP.SSM.MISSING.001[​](#ctleksnodegroupssmmissing001 "Direct link to CTL.EKS.NODEGROUP.SSM.MISSING.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: AC-3, AU-12; iso\_27001\_2022: A.5.15, A.8.15; nist\_800\_53\_r5: AC-3, AU-12, IA-2; soc2: CC6.1, CC7.2;
...
### CTL.EKS.NS.DORMANT.001[​](#ctleksnsdormant001 "Direct link to CTL.EKS.NS.DORMANT.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: CM-2, CM-8; iso\_27001\_2022: A.5.16, A.8.32; nist\_800\_53\_r5: CM-2, CM-8; soc2: CC8.1;
...
**Remediation:** Delete the namespace via kubectl delete ns . If the namespace must be retained for a documented reason (capacity reservation, naming reservation), tag it with Lifecycle=Reserved + DocumentedReason=. Audit cluster namespace inventory periodically via kubectl get ns plus the `kubernetes.io/cluster-event-history` audit log.

***

### CTL.EKS.NS.MULTITENANT.001[​](#ctleksnsmultitenant001 "Direct link to CTL.EKS.NS.MULTITENANT.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: AC-3, AC-4; iso\_27001\_2022: A.5.15, A.8.20; nist\_800\_53\_r5: AC-3, AC-4; soc2: CC6.1, CC6.6;
...
### CTL.EKS.NS.NOCLASSIFICATION.TAG.001[​](#ctleksnsnoclassificationtag001 "Direct link to CTL.EKS.NS.NOCLASSIFICATION.TAG.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: CA-7, CM-8; iso\_27001\_2022: A.5.9, A.5.12; nist\_800\_53\_r5: CA-7, CM-8; soc2: CC6.1, CC6.5;
...
**Remediation:** Apply org-standard classification labels: kubectl label ns data-classification=internal compliance=hipaa owner=team-name Standardize the label vocabulary across the cluster (Public / Internal / Confidential / Restricted is one common scheme). Compliance reports can then scope by label: kubectl get ns -l data-classification=restricted

***

### CTL.EKS.NS.NOCOSTALLOC.TAG.001[​](#ctleksnsnocostalloctag001 "Direct link to CTL.EKS.NS.NOCOSTALLOC.TAG.001")
...
**Remediation:** Apply org-standard labels: kubectl label ns cost-center=engineering-billing team=billing owner=<billing-leads@example.com> Consider AWS Cost Allocation Tags (activate via Billing Console) so per-namespace costs roll up to AWS Cost Explorer. EKS-specific tools like Kubecost / CloudWatch Container Insights consume namespace labels directly for chargeback reports.

***

### CTL.EKS.NS.NOLIMITRANGE.001[​](#ctleksnsnolimitrange001 "Direct link to CTL.EKS.NS.NOLIMITRANGE.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: SC-5; iso\_27001\_2022: A.8.6; nist\_800\_53\_r5: SC-5, SC-6; soc2: CC8.1, A1.1;
...
**Remediation:** Apply a LimitRange to enforce default + max per-pod / per-container resource bounds: apiVersion: v1 kind: LimitRange metadata: { name: -limits, namespace: } spec: limits: - type: Container default: { cpu: 500m, memory: 512Mi } defaultRequest: { cpu: 100m, memory: 128Mi } max: { cpu: 2, memory: 4Gi } min: { cpu: 50m, memory: 64Mi } Tune to the workload's expected per-pod range.

***

### CTL.EKS.NS.NORESOURCEQUOTA.001[​](#ctleksnsnoresourcequota001 "Direct link to CTL.EKS.NS.NORESOURCEQUOTA.001")
...
**Remediation:** Apply a ResourceQuota matching the namespace's intended resource budget: apiVersion: v1 kind: ResourceQuota metadata: name: -quota namespace: spec: hard: requests.cpu: "20" requests.memory: 40Gi limits.cpu: "40" limits.memory: 80Gi pods: "100" Tune the values to the workload's expected steady-state plus headroom.

***

### CTL.EKS.NS.PRIORITYCLASS.SYSTEM.001[​](#ctleksnspriorityclasssystem001 "Direct link to CTL.EKS.NS.PRIORITYCLASS.SYSTEM.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: AC-3, SC-5; iso\_27001\_2022: A.5.15, A.8.6; nist\_800\_53\_r5: AC-3, SC-5; soc2: CC6.1, CC6.6, CC8.1;
...
### CTL.EKS.OIDC.SHARED.001[​](#ctleksoidcshared001 "Direct link to CTL.EKS.OIDC.SHARED.001")
...
* **Type:** unsafe\_state
* **Domain:** identity
* **Compliance:** fedramp\_moderate: AC-3, AC-6; iso\_27001\_2022: A.5.15, A.8.2; nist\_800\_53\_r5: AC-3, AC-6, IA-5; soc2: CC6.1, CC6.3;
...
**Remediation:** Verify each cluster's OIDC issuer URL is unique: aws eks describe-cluster --name --query 'cluster.identity.oidc.issuer' If two clusters share an URL, this is a configuration error — recreate the affected cluster (the OIDC URL is set at cluster creation and can't be changed). Re-deploy IRSA roles afterward with the new OIDC URLs.

***

### CTL.EKS.POD.GHOST.CONFIGMAP.001[​](#ctlekspodghostconfigmap001 "Direct link to CTL.EKS.POD.GHOST.CONFIGMAP.001")
...
**Remediation:** Recreate the ConfigMap at the original name (kubectl create configmap), or update the pod spec to reference a live ConfigMap (kubectl edit deploy ). Audit via `kubectl get pods -A -o yaml | grep configMapRef`.

***

### CTL.EKS.POD.GHOST.SECRET.001[​](#ctlekspodghostsecret001 "Direct link to CTL.EKS.POD.GHOST.SECRET.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: CM-2; iso\_27001\_2022: A.5.16, A.8.32; nist\_800\_53\_r5: CM-2, CM-3; soc2: CC6.1, CC8.1, CC7.4;
...
**Remediation:** Recreate the Secret at the original name (kubectl create secret), or update the pod spec to reference a live Secret (kubectl edit deploy ). Audit pods for ghost Secret refs via `kubectl get pods -A -o yaml | grep secretRef` plus a kubectl get secret -A walk.

***

### CTL.EKS.POD.HOSTACCESS.001[​](#ctlekspodhostaccess001 "Direct link to CTL.EKS.POD.HOSTACCESS.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: AC-3, AC-6; iso\_27001\_2022: A.5.15, A.8.16; nist\_800\_53\_r5: AC-3, AC-6, SC-7; pci\_dss\_v4.0: 1.4; soc2: CC6.1, CC6.6;
...
### CTL.EKS.POD.HOSTPATH.001[​](#ctlekspodhostpath001 "Direct link to CTL.EKS.POD.HOSTPATH.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: AC-3, AC-6; iso\_27001\_2022: A.5.15, A.8.16; nist\_800\_53\_r5: AC-3, AC-6, SC-7; pci\_dss\_v4.0: 1.4, 2.2; soc2: CC6.1, CC6.6;
...
### CTL.EKS.POD.NODE.PROFILE.INHERIT.001[​](#ctlekspodnodeprofileinherit001 "Direct link to CTL.EKS.POD.NODE.PROFILE.INHERIT.001")
...
* **Type:** unsafe\_state
* **Domain:** identity
* **Compliance:** fedramp\_moderate: AC-3, AC-6; iso\_27001\_2022: A.5.15, A.8.2; nist\_800\_53\_r5: AC-3, AC-6, IA-2; pci\_dss\_v4.0: 7.2; soc2: CC6.1, CC6.3;
...
### CTL.EKS.POD.PRIVESCALATION.001[​](#ctlekspodprivescalation001 "Direct link to CTL.EKS.POD.PRIVESCALATION.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: AC-3, AC-6; iso\_27001\_2022: A.5.15, A.8.16; nist\_800\_53\_r5: AC-3, AC-6; pci\_dss\_v4.0: 2.2; soc2: CC6.1, CC6.6;
...
### CTL.EKS.POD.PRIVILEGED.001[​](#ctlekspodprivileged001 "Direct link to CTL.EKS.POD.PRIVILEGED.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: AC-3, AC-6; iso\_27001\_2022: A.5.15, A.8.2, A.8.16; nist\_800\_53\_r5: AC-3, AC-6, SI-3; pci\_dss\_v4.0: 1.4, 2.2; soc2: CC6.1, CC6.6;
...
### CTL.EKS.POD.RUNASROOT.001[​](#ctlekspodrunasroot001 "Direct link to CTL.EKS.POD.RUNASROOT.001")
...
### CTL.EKS.POD.SATOKEN.AUTOMOUNT.001[​](#ctlekspodsatokenautomount001 "Direct link to CTL.EKS.POD.SATOKEN.AUTOMOUNT.001")
...
### CTL.EKS.PODIDENTITY.DEFAULTNS.001[​](#ctlekspodidentitydefaultns001 "Direct link to CTL.EKS.PODIDENTITY.DEFAULTNS.001")
...
* **Type:** unsafe\_state
* **Domain:** identity
* **Compliance:** fedramp\_moderate: AC-3; iso\_27001\_2022: A.5.15, A.8.20; nist\_800\_53\_r5: AC-3, AC-4; soc2: CC6.1, CC6.6;
...
### CTL.EKS.PODIDENTITY.GHOST.SA.001[​](#ctlekspodidentityghostsa001 "Direct link to CTL.EKS.PODIDENTITY.GHOST.SA.001")
...
### CTL.EKS.PSA.AUDITONLY.001[​](#ctlekspsaauditonly001 "Direct link to CTL.EKS.PSA.AUDITONLY.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: AC-3, SI-3; iso\_27001\_2022: A.5.15, A.8.16; nist\_800\_53\_r5: AC-3, CM-2, SI-3; soc2: CC6.1, CC8.1;
...
**Remediation:** Promote audit / warn to enforce after a measurement period (typically 2-4 weeks of audit events review). Set: pod-security.kubernetes.io/enforce=restricted leaving audit / warn at the same level for cross-check. If the namespace has legitimate violations at the chosen level, exempt them via per-pod annotations rather than weakening the namespace-wide enforce.

***

### CTL.EKS.PSA.MISSING.001[​](#ctlekspsamissing001 "Direct link to CTL.EKS.PSA.MISSING.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: AC-3, SI-3; iso\_27001\_2022: A.5.15, A.8.16; nist\_800\_53\_r5: AC-3, CM-2, SI-3; pci\_dss\_v4.0: 1.4, 2.2; soc2: CC6.1, CC6.6;
...
**Remediation:** Apply a PSA enforce label appropriate to the workload profile. For production application namespaces: kubectl label ns<br />pod-security.kubernetes.io/enforce=restricted<br />pod-security.kubernetes.io/audit=restricted<br />pod-security.kubernetes.io/warn=restricted For workloads needing elevated privileges, use `baseline` rather than `privileged` and document the reason in the namespace's annotations.

***

### CTL.EKS.PSA.PRIVILEGED.PROD.001[​](#ctlekspsaprivilegedprod001 "Direct link to CTL.EKS.PSA.PRIVILEGED.PROD.001")
...
### CTL.EKS.PUBLIC.ENDPOINT.001[​](#ctlekspublicendpoint001 "Direct link to CTL.EKS.PUBLIC.ENDPOINT.001")
...
* **Type:** unsafe\_state
* **Domain:** exposure
* **Compliance:** cis\_eks: 2.2; fedramp\_moderate: SC-7; nist\_800\_53\_r5: AC-17;
...
**Remediation:** Option 1 (preferred): Disable public endpoint entirely. Option 2: Restrict to specific CIDRs (corporate NAT, VPN, CI/CD). aws eks update-cluster-config --name --resources-vpc-config endpointPublicAccess=true, publicAccessCidrs="10.0.0.0/8,203.0.113.0/24"

***

### CTL.EKS.PV.RELEASED.STALE.001[​](#ctlekspvreleasedstale001 "Direct link to CTL.EKS.PV.RELEASED.STALE.001")
...
### CTL.EKS.PVC.GHOST.STORAGECLASS.001[​](#ctlekspvcghoststorageclass001 "Direct link to CTL.EKS.PVC.GHOST.STORAGECLASS.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: CM-2; iso\_27001\_2022: A.5.16, A.8.13, A.8.32; nist\_800\_53\_r5: CM-2, CM-3; soc2: CC8.1, A1.1, CC7.4;
...
### CTL.EKS.RAM.EXTERNAL.001[​](#ctleksramexternal001 "Direct link to CTL.EKS.RAM.EXTERNAL.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: AC-3, SC-7; iso\_27001\_2022: A.5.15, A.8.20; nist\_800\_53\_r5: AC-3, AC-4, SC-7; pci\_dss\_v4.0: 1.2, 7.1; soc2: CC6.1, CC6.6;
...
**Remediation:** Restrict RAM share to accounts within the org by adding `aws:PrincipalOrgID` condition. For legitimately external collaborators, use a dedicated cluster (per-tenant clusters) rather than RAM-share. Audit current RAM shares: aws ram get-resource-share-associations --association-type RESOURCE and remove any non-org principals.

***

### CTL.EKS.RBAC.AUDIT.001[​](#ctleksrbacaudit001 "Direct link to CTL.EKS.RBAC.AUDIT.001")
...
* **Type:** unsafe\_state
* **Domain:** exposure
* **Compliance:** cis\_k8s\_v1.9: 5.1.1; nist\_800\_53\_r5: AC-6;
...
### CTL.EKS.RBAC.GHOST.ROLEREF.001[​](#ctleksrbacghostroleref001 "Direct link to CTL.EKS.RBAC.GHOST.ROLEREF.001")
...
**Remediation:** Delete the orphan binding via kubectl delete rolebinding / clusterrolebinding . If the role name will be reused, audit who the binding granted to BEFORE deletion to confirm the intended grants.

***

### CTL.EKS.RBAC.GHOST.SUBJECT.001[​](#ctleksrbacghostsubject001 "Direct link to CTL.EKS.RBAC.GHOST.SUBJECT.001")
...
**Remediation:** Delete the binding (kubectl delete rolebinding / clusterrolebinding ) OR remove the orphan subject from the subjects list. Audit cluster bindings via `kubectl get clusterrolebinding,rolebinding -A -o yaml` and verify every subject resolves: ServiceAccounts via kubectl get sa, Users / Groups via the cluster's auth provider.

***

### CTL.EKS.SECRETS.ENCRYPT.001[​](#ctlekssecretsencrypt001 "Direct link to CTL.EKS.SECRETS.ENCRYPT.001")
...
* **Type:** unsafe\_state
* **Domain:** exposure
* **Compliance:** cis\_eks: 2.3; fedramp\_moderate: SC-28; hipaa: 164.312(a)(2)(iv); nist\_800\_53\_r5: SC-28;
...
**Remediation:** Enable secrets encryption: aws eks associate-encryption-config --cluster-name --encryption-config '\[{"resources":\["secrets"],"provider": {"keyArn":"arn:aws:kms:::key/"}}]'

***

### CTL.EKS.SECRETS.ROTATION.001[​](#ctlekssecretsrotation001 "Direct link to CTL.EKS.SECRETS.ROTATION.001")
...
* **Type:** unsafe\_state
* **Domain:** encryption
* **Compliance:** mitre\_attack: TA0006; nist\_800\_53\_r5: SC-12; owasp\_nhi: NHI7;
...
**Remediation:** Enable automatic rotation on the KMS key: aws kms enable-key-rotation --key-id

***

### CTL.EKS.SERVICE.GHOST.SG.001[​](#ctleksserviceghostsg001 "Direct link to CTL.EKS.SERVICE.GHOST.SG.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: CM-2, AC-3; iso\_27001\_2022: A.5.15, A.5.16, A.8.32; nist\_800\_53\_r5: CM-2, CM-3, AC-3; soc2: CC6.1, CC8.1, CC7.4;
...
**Remediation:** Update the Service's `service.beta.kubernetes.io/aws-load-balancer-security-groups` annotation to a live SG ID (kubectl edit svc ), or recreate the SG at the original ID if reproducible. Pair with VPC SG ghost detection (CTL.VPC.SG.GHOST.001) to catch the SG-deletion event earlier.

***

### CTL.EKS.SERVICE.LB.NOENDPOINTS.001[​](#ctleksservicelbnoendpoints001 "Direct link to CTL.EKS.SERVICE.LB.NOENDPOINTS.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: CM-2; iso\_27001\_2022: A.5.16, A.8.16; nist\_800\_53\_r5: CM-2, CM-8; soc2: CC8.1, A1.1;
...
**Remediation:** Diagnose root cause: Service selector vs pod labels mismatch (kubectl get pods -l ); workloads CrashLooping (kubectl get pods); workloads scaled to zero. Either fix the underlying workload, update the Service selector, or delete the Service if the workload is decommissioned (also tear down the ELB AWS cost).

***

### CTL.EKS.STORAGECLASS.GHOST.KMS.001[​](#ctleksstorageclassghostkms001 "Direct link to CTL.EKS.STORAGECLASS.GHOST.KMS.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: CM-2, SC-12; iso\_27001\_2022: A.5.16, A.8.24, A.8.32; nist\_800\_53\_r5: CM-2, CM-3, SC-12; soc2: CC6.1, CC8.1, CC7.4;
...
### CTL.EKS.STORAGECLASS.GP2DEFAULT.001[​](#ctleksstorageclassgp2default001 "Direct link to CTL.EKS.STORAGECLASS.GP2DEFAULT.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: CM-2; iso\_27001\_2022: A.5.16; nist\_800\_53\_r5: CM-2; soc2: CC8.1, A1.1;
...
### CTL.EKS.STORAGECLASS.NOTENCRYPTED.001[​](#ctleksstorageclassnotencrypted001 "Direct link to CTL.EKS.STORAGECLASS.NOTENCRYPTED.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: SC-12, SC-28; iso\_27001\_2022: A.8.24; nist\_800\_53\_r5: SC-12, SC-28; pci\_dss\_v4.0: 3.5; soc2: CC6.1, CC6.7;
...
**Remediation:** Update StorageClass parameters to include encrypted: "true" plus optional kmsKeyId for CMK encryption: apiVersion: storage.k8s.io/v1 kind: StorageClass metadata: { name: gp3-encrypted } provisioner: ebs.csi.aws.com parameters: type: gp3 encrypted: "true" kmsKeyId: "arn:aws:kms:us-east-1:111122223333:key/abc" For new clusters, also enable EBS account-level default encryption as defense in depth.

***

### CTL.EKS.STORAGECLASS.NOWAITFORCONSUMER.001[​](#ctleksstorageclassnowaitforconsumer001 "Direct link to CTL.EKS.STORAGECLASS.NOWAITFORCONSUMER.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: CM-2; iso\_27001\_2022: A.5.16, A.8.13; nist\_800\_53\_r5: CM-2; soc2: CC8.1, A1.1;
...
**Remediation:** Update the StorageClass to use volumeBindingMode: WaitForFirstConsumer: apiVersion: storage.k8s.io/v1 kind: StorageClass metadata: { name: gp3 } provisioner: ebs.csi.aws.com volumeBindingMode: WaitForFirstConsumer Existing PVs are unaffected; new PVCs get the new binding behavior. WaitForFirstConsumer is the Kubernetes-recommended default for cloud-provider storage that's AZ-bound.

***

### CTL.EKS.VELERO.RESTORE.SCOPE.001[​](#ctleksvelerorestorescope001 "Direct link to CTL.EKS.VELERO.RESTORE.SCOPE.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: AC-3, CP-9; iso\_27001\_2022: A.5.15, A.8.13; nist\_800\_53\_r5: AC-3, AC-4, CP-9; soc2: CC6.1, CC6.6, A1.2;
...
### CTL.EKS.VERSION.001[​](#ctleksversion001 "Direct link to CTL.EKS.VERSION.001")
...
* **Type:** unsafe\_state
* **Domain:** exposure
* **Compliance:** fedramp\_moderate: CM-6; hipaa: 164.312(a)(2)(i); nist\_800\_53\_r5: CM-6; pci\_dss\_v4.0: 2.2.1; soc2: CC7.1;
...
### CTL.EKS.VPC.CNI.CUSTOMNETWORKING.001[​](#ctleksvpccnicustomnetworking001 "Direct link to CTL.EKS.VPC.CNI.CUSTOMNETWORKING.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: CM-2, SC-7; iso\_27001\_2022: A.5.16, A.8.20; nist\_800\_53\_r5: CM-2, AC-4, SC-7; soc2: CC6.1, CC8.1;

EKS VPC CNI add-on has AWS\_VPC\_K8S\_CNI\_CUSTOM\_NETWORK\_CFG=true (custom networking enabled) but no ENIConfig CRDs are created in the cluster. Pods get IPs from the default subnet ranges instead of the custom per-AZ subnets the operator intended to provision. Multi-tenant clusters depending on custom networking for IP isolation lose the isolation; pods land in the wrong subnets.

**Remediation:** Create ENIConfig CRDs per AZ, each pointing at the custom subnet for that AZ: apiVersion: crd.k8s.amazonaws.com/v1alpha1 kind: ENIConfig metadata: { name: us-east-1a } spec: securityGroups: \[sg-...] subnet: subnet-... Then label each node with its AZ's ENIConfig via the VPC CNI's ENI\_CONFIG\_LABEL\_DEF environment variable. Verify pod IPs land in the custom subnets via `kubectl get pods -A -o wide`.

***

### CTL.EKS.VPC.CNI.NETPOL.TTL.001[​](#ctleksvpccninetpolttl001 "Direct link to CTL.EKS.VPC.CNI.NETPOL.TTL.001")
...
**Remediation:** Option 1: Enable the TTLAfterFinished feature gate at cluster level. Option 2: Set ttlSecondsAfterFinished on all Job specs. Option 3: Update VPC CNI to a version with pod completion handling. Verify VPC CNI version: kubectl describe daemonset aws-node -n kube-system | grep Image

***

### CTL.EKS.WORKLOAD.GHOST.IMAGE.001[​](#ctleksworkloadghostimage001 "Direct link to CTL.EKS.WORKLOAD.GHOST.IMAGE.001")
...
* **Type:** unsafe\_state
* **Domain:** governance
* **Compliance:** fedramp\_moderate: CM-2, SI-4; iso\_27001\_2022: A.5.16, A.8.32; nist\_800\_53\_r5: CM-2, CM-3, SI-4; soc2: CC6.1, CC8.1, CC7.4;
...
**Remediation:** Either recreate the ECR repo + republish the expected image, or update the workload spec to point at a live repository (kubectl set image deploy/ container=). For the long term: add an ECR repository policy that prevents accidental deletion plus a CloudWatch alarm on DeleteRepository events.
