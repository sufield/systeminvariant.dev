...
**ID:** `CTL.S3.LIFECYCLE.002` **Category:** Lifecycle and Governance **Severity:** Critical

## What This Checks[​](#what-this-checks "Direct link to What This Checks")

S3 buckets tagged with `data-classification=phi` must not have lifecycle expiration rules that delete objects before 2190 days (6 years). This control fires when a PHI bucket has an expiration rule with `min_expiration_days` below that threshold.
...
```
{
  "control_id": "CTL.S3.LIFECYCLE.002",
  "control_name": "PHI Buckets Must Not Expire Data Before Minimum Retention",
...
* [`CTL.S3.LIFECYCLE.001`](/docs/reference/invariants-reference/lifecycle-governance/inv-s3-lifecycle-001.md) -- Ensures retention-tagged buckets have lifecycle rules at all; this control checks the rules are long enough.
* [`CTL.S3.LOCK.003`](/docs/reference/invariants-reference/lifecycle-governance/inv-s3-lock-003.md) -- Enforces the same 2190-day minimum on Object Lock retention for PHI buckets.
* [`CTL.S3.LOCK.002`](/docs/reference/invariants-reference/lifecycle-governance/inv-s3-lock-002.md) -- Requires COMPLIANCE mode Object Lock for PHI, complementing lifecycle retention with WORM protection.
