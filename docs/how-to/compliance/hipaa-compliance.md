...
1. **HIPAA Pack** (`stave apply --profile hipaa`) — evaluates 11 declarative YAML controls against S3 observations using the standard evaluation pipeline with duration-aware violation tracking.
2. **HIPAA Profile** (`stave evaluate --profile hipaa`) — runs programmatic Go invariants with compound risk detection, acknowledged exceptions, and structured compliance reporting.

Both approaches map to the same HIPAA Security Rule technical safeguards but serve different workflows.

## Quick Start[​](#quick-start "Direct link to Quick Start")

### Using the HIPAA Pack (Standard Pipeline)[​](#using-the-hipaa-pack-standard-pipeline "Direct link to Using the HIPAA Pack (Standard Pipeline)")
...
The HIPAA pack selects 28 controls from the IAM and S3 catalogs that map to HIPAA technical safeguards. Use `--max-unsafe 0s` for zero tolerance on PHI exposure.

### Using the HIPAA Profile (Compliance Reporting)[​](#using-the-hipaa-profile-compliance-reporting "Direct link to Using the HIPAA Profile (Compliance Reporting)")
...
## HIPAA Technical Safeguards Mapping[​](#hipaa-technical-safeguards-mapping "Direct link to HIPAA Technical Safeguards Mapping")

### Access Control (45 CFR 164.312(a)(1))[​](#access-control-45-cfr-164312a1 "Direct link to Access Control (45 CFR 164.312(a)(1))")
...
### Encryption (45 CFR 164.312(a)(2)(iv), 164.312(e))[​](#encryption-45-cfr-164312a2iv-164312e "Direct link to Encryption (45 CFR 164.312(a)(2)(iv), 164.312(e))")
...
### Audit Controls (45 CFR 164.312(b))[​](#audit-controls-45-cfr-164312b "Direct link to Audit Controls (45 CFR 164.312(b))")
...
### Governance & Retention[​](#governance--retention "Direct link to Governance & Retention")
...
## Acknowledged Exceptions[​](#acknowledged-exceptions "Direct link to Acknowledged Exceptions")

When a control cannot be remediated immediately, declare an exception in `stave.yaml` next to the snapshot:
...
The exception is valid only if all compensating controls in `requires_passing` are passing. Otherwise the finding remains with a note that the compensating control is not met.

## S3 Control Mapping (Standard Pipeline)[​](#s3-control-mapping-standard-pipeline "Direct link to S3 Control Mapping (Standard Pipeline)")

The standard `stave apply` pipeline uses YAML-based controls from the S3 catalog. These map to HIPAA requirements:
...
## Report Output[​](#report-output "Direct link to Report Output")

### Text Report[​](#text-report "Direct link to Text Report")
...
### JSON Report[​](#json-report "Direct link to JSON Report")

```
stave evaluate --snapshot snap.json --profile hipaa --format json | jq '.summary'
```

Returns structured JSON with `results`, `compound_findings`, `acknowledged`, and `counts` sections.

## Limitations[​](#limitations "Direct link to Limitations")

* Stave evaluates **technical controls only**. A BAA with AWS is a contractual prerequisite for HIPAA compliance that Stave cannot verify.
* Controls that read CloudTrail or VPC endpoint data (AUDIT.002, ACCESS.006) require extractors that call `aws cloudtrail get-event-selectors` and `aws ec2 describe-vpc-endpoints` respectively. If these observation fields are absent, the controls fail with a message about what data is needed.
* Some HIPAA obligations (log review processes, malware scanning, minimum necessary access) are process or multi-service requirements that cannot be proven from S3 bucket configuration alone.
