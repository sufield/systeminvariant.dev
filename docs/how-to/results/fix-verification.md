...
A finding is potential value. A closed finding is realized value. This page shows the loop: finding → fix → re-snapshot → `stave diff` → confirmed closed.

## The loop[​](#the-loop "Direct link to The loop")

### 1. You have a finding[​](#1-you-have-a-finding "Direct link to 1. You have a finding")
...
### 2. Fix it[​](#2-fix-it "Direct link to 2. Fix it")

Enable versioning on the bucket (console, CLI, Terraform — whatever your change process is).

### 3. Re-capture the snapshot[​](#3-re-capture-the-snapshot "Direct link to 3. Re-capture the snapshot")
...
### 4. Run `stave diff`[​](#4-run-stave-diff "Direct link to 4-run-stave-diff")
...
### 5. Read the result[​](#5-read-the-result "Direct link to 5. Read the result")
...
## Formal closure with `stave prove`[​](#formal-closure-with-stave-prove "Direct link to formal-closure-with-stave-prove")
...
If the result is `UNSAT`, the forbidden state is provably absent from the current configuration — not just "not found" but mathematically impossible given the observed settings.
...
See [Running in CI/CD](/docs/how-to/integration/ci-cd-integration.md) for pipeline configuration.
