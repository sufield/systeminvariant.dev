...
## Anatomy of a finding[​](#anatomy-of-a-finding "Direct link to Anatomy of a finding")
...
## Fast triage checklist[​](#fast-triage-checklist "Direct link to Fast triage checklist")
...
## Show one fix plan in terminal[​](#show-one-fix-plan-in-terminal "Direct link to Show one fix plan in terminal")
...
## Zero findings: what it means[​](#zero-findings-what-it-means "Direct link to Zero findings: what it means")

A clean run — zero findings — is not an empty result. It is a **negative-assurance artifact**: the controls were evaluated, the services were covered, and no unsafe state was found.

In audit and diligence framing, a clean report is a positive deliverable:

* **Controls evaluated** — the full catalog (or template scope) ran
* **Services covered** — every resource type in the snapshot was checked against applicable controls
* **Assessment scope** — the snapshot's resource count, account, and capture timestamp are recorded

A clean report combined with an [evidence packet](/docs/reference/evidence-packet-spec.md) is the artifact an auditor or CISO receives. "No findings" backed by a manifest and catalog version is a stronger statement than "we didn't check."

## When results look unexpected[​](#when-results-look-unexpected "Direct link to When results look unexpected")
