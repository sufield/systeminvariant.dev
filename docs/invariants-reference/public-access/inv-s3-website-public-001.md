...
sidebar_label: "INV.S3.WEBSITE.PUBLIC.001"
sidebar_position: 10
...
**ID:** `INV.S3.WEBSITE.PUBLIC.001`
**Category:** Public Access
**Severity:** High

## What This Checks

S3 buckets with static website hosting enabled must not also have public read access. This invariant fires when `website.enabled` is `true` and `public_read` is `true`, meaning the bucket serves content directly to the internet via the S3 website endpoint without any CDN or access control in front of it.
...
```json
{
  "invariant_id": "INV.S3.WEBSITE.PUBLIC.001",
  "invariant_name": "No Public Website Hosting with Public Read",
...
- [`INV.S3.PUBLIC.001`](inv-s3-public-001.md) -- General check for public read access; this invariant adds the website-hosting dimension.
- [`INV.S3.PUBLIC.005`](inv-s3-public-005.md) -- Detects latent public read exposure, which is especially dangerous for website-hosting buckets.
- [`INV.S3.CONTROLS.001`](inv-s3-controls-001.md) -- Ensures Public Access Block is enabled as defense-in-depth for website-hosting buckets.
