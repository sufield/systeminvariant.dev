---
title: "Anonymous S3 Listing Must Be Explicitly Intended"
sidebar_label: "INV.S3.PUBLIC.LIST.002"
sidebar_position: 8
description: "Anonymous bucket listing increases exposure surface and must be explicitly tagged as intended."
---

# Anonymous S3 Listing Must Be Explicitly Intended

**ID:** `INV.S3.PUBLIC.LIST.002`
**Category:** Public Access
**Severity:** Medium

## What This Checks

Anonymous bucket listing increases exposure surface even when objects are public by design. This invariant fires when a bucket has `public_list` set to `true` but does not carry the tag `public_list_intended=true`. It enforces an explicit opt-in for directory enumeration on public-content buckets.
...
```json
{
  "invariant_id": "INV.S3.PUBLIC.LIST.002",
  "invariant_name": "Anonymous S3 Listing Must Be Explicitly Intended",
...
Alternatively, a bucket with `public_list` set to `false` also passes this invariant.

## Related Invariants

- [`INV.S3.PUBLIC.LIST.001`](inv-s3-public-list-001.md) -- Unconditionally blocks public listing; this invariant is the softer variant that allows listing with an intent tag.
- [`INV.S3.PUBLIC.006`](inv-s3-public-006.md) -- Detects latent public listing masked only by Public Access Block.
- [`INV.S3.PUBLIC.001`](inv-s3-public-001.md) -- General public access check that also covers listing.
