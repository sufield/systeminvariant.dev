...
S3 buckets tagged with `data-classification=phi` that have Object Lock enabled must use COMPLIANCE mode. This invariant fires when a PHI bucket's Object Lock mode is set to anything other than `COMPLIANCE` (typically `GOVERNANCE`).
...
```json
{
  "invariant_id": "INV.S3.LOCK.002",
  "invariant_name": "PHI Buckets Must Use COMPLIANCE Mode Object Lock",
...
- [`INV.S3.LOCK.001`](inv-s3-lock-001.md) -- Requires Object Lock to be enabled on compliance-tagged buckets; this invariant checks the mode is correct.
- [`INV.S3.LOCK.003`](inv-s3-lock-003.md) -- Enforces that the COMPLIANCE mode retention period meets the 2190-day HIPAA minimum.
- [`INV.S3.LIFECYCLE.002`](inv-s3-lifecycle-002.md) -- Enforces the same 2190-day minimum on lifecycle expiration rules for PHI buckets.
