...
sidebar_label: "INV.S3.VERSION.001"
sidebar_position: 4
...
**ID:** `INV.S3.VERSION.001`
**Category:** Lifecycle and Governance
**Severity:** Medium

## What This Checks

S3 buckets must have versioning enabled. This invariant fires when any bucket has `versioning.enabled` set to `false`.
...
```json
{
  "invariant_id": "INV.S3.VERSION.001",
  "invariant_name": "Versioning Required",
...
- [`INV.S3.VERSION.002`](inv-s3-version-002.md) -- Requires MFA delete on backup-tagged buckets; MFA delete depends on versioning being enabled.
- [`INV.S3.LOCK.001`](inv-s3-lock-001.md) -- Object Lock requires versioning as a prerequisite.
- [`INV.S3.LIFECYCLE.001`](inv-s3-lifecycle-001.md) -- Lifecycle rules for noncurrent version expiration complement versioning to control storage costs.
