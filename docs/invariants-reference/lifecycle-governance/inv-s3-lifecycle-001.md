...
sidebar_label: "INV.S3.LIFECYCLE.001"
sidebar_position: 2
...
**ID:** `INV.S3.LIFECYCLE.001`
**Category:** Lifecycle and Governance
**Severity:** Medium

## What This Checks

S3 buckets that carry a `data-retention` tag must have at least one enabled lifecycle rule configured. This invariant fires when a bucket declares a retention period via tag but has no lifecycle rules to enforce it.
...
```json
{
  "invariant_id": "INV.S3.LIFECYCLE.001",
  "invariant_name": "Retention-Tagged Buckets Must Have Lifecycle Rules",
...
- [`INV.S3.LIFECYCLE.002`](inv-s3-lifecycle-002.md) -- Enforces that PHI buckets do not expire data before the 2190-day HIPAA minimum.
- [`INV.S3.GOVERNANCE.001`](inv-s3-governance-001.md) -- Requires the `data-classification` tag that often accompanies retention tagging.
- [`INV.S3.VERSION.001`](inv-s3-version-001.md) -- Versioning complements lifecycle rules by preserving previous versions before expiration.
