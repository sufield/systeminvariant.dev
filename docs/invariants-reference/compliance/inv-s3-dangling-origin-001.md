...
sidebar_label: "INV.S3.DANGLING.ORIGIN.001"
sidebar_position: 2
...
**ID:** `INV.S3.DANGLING.ORIGIN.001`
**Category:** Compliance
**Severity:** Critical

## What This Checks

CloudFront distributions must not reference S3 origin buckets that do not exist. This invariant fires when `cdn.kind` is `"distribution"` and `origins_has_dangling_s3` is `true`.
...
```json
{
  ...
  "findings": [
    {
      "invariant_id": "INV.S3.DANGLING.ORIGIN.001",
...
- [`INV.S3.BUCKET.TAKEOVER.001`](./inv-s3-bucket-takeover-001.md) -- General-purpose check that any referenced S3 bucket exists and is owned.
- [`INV.S3.CONTROLS.001`](../public-access/inv-s3-controls-001.md) -- Ensures Public Access Block is fully enabled on origin buckets.
