...
sidebar_label: "INV.S3.BUCKET.TAKEOVER.001"
sidebar_position: 1
...
**ID:** `INV.S3.BUCKET.TAKEOVER.001`
**Category:** Compliance
**Severity:** Critical

## What This Checks

Any S3 bucket referenced by a DNS CNAME, CDN origin, or application configuration must exist and be owned by your AWS account. This invariant fires when `s3_ref.bucket_exists` is `false` or `s3_ref.bucket_owned` is `false`.
...
```json
{
  ...
  "findings": [
    {
      "invariant_id": "INV.S3.BUCKET.TAKEOVER.001",
...
- [`INV.S3.DANGLING.ORIGIN.001`](./inv-s3-dangling-origin-001.md) -- CDN-specific variant that checks CloudFront distributions for dangling S3 origins.
- [`INV.S3.CONTROLS.001`](../public-access/inv-s3-controls-001.md) -- Ensures Public Access Block is fully enabled as defense-in-depth.
