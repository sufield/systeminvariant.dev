---
title: "Shared-Bucket Tenant Isolation Must Enforce Prefix"
sidebar_label: "INV.S3.TENANT.ISOLATION.001"
sidebar_position: 6
description: "When a shared S3 bucket uses prefix-based tenant isolation, every app-signer identity must enforce the tenant prefix."
---

# Shared-Bucket Tenant Isolation Must Enforce Prefix

**ID:** `INV.S3.TENANT.ISOLATION.001`
...
```json
{
  "invariant_id": "INV.S3.TENANT.ISOLATION.001",
  "invariant_name": "Shared-Bucket Tenant Isolation Must Enforce Prefix",
...
- [`INV.S3.WRITE.SCOPE.001`](./inv-s3-write-scope-001.md) -- S3 Signed Upload Must Bind To Exact Object Key (flags prefix-wide write grants in upload policies)
- [`INV.S3.ACCESS.003`](./inv-s3-access-003.md) -- No External Write Access (flags external write grants that compound tenant isolation failures)
- [`INV.S3.ACCESS.001`](./inv-s3-access-001.md) -- No Unauthorized Cross-Account Access (flags external account access that could bypass tenant boundaries)
