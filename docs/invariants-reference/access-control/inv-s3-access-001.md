---
title: "No Unauthorized Cross-Account Access"
sidebar_label: "INV.S3.ACCESS.001"
sidebar_position: 1
description: "S3 bucket policies must not grant access to external AWS accounts."
---

# No Unauthorized Cross-Account Access

**ID:** `INV.S3.ACCESS.001`
**Category:** Access Control
**Severity:** High

## What This Checks

S3 bucket policies must not grant access to external AWS accounts not in the `allowed_accounts` list. The invariant checks `external_account_ids` against a configurable allowlist and fires when any account ID is not in the permitted set.
...
```json
{
  "invariant_id": "INV.S3.ACCESS.001",
  "invariant_name": "No Unauthorized Cross-Account Access",
...
A safe bucket has no external account IDs, or only those listed in the invariant's `allowed_accounts` parameter:
...
**To remediate:** Review bucket policy `Principal` elements for external account IDs. Remove statements granting access to accounts not in your organization. Use the `aws:PrincipalOrgID` condition key to restrict access to your AWS Organization. Alternatively, add trusted account IDs to the invariant's `allowed_accounts` parameter.

## Related Invariants

- [`INV.S3.ACCESS.003`](./inv-s3-access-003.md) -- No External Write Access (stricter: flags write/delete grants to external accounts)
- [`INV.S3.NETWORK.001`](./inv-s3-network-001.md) -- Public-Principal Policies Must Have Network Conditions (flags `Principal: *` without network scoping)
- [`INV.S3.ACCESS.002`](./inv-s3-access-002.md) -- No Wildcard Action Policies (flags overly broad action grants)
