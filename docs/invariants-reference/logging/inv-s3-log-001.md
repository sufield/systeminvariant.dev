...
S3 buckets must have server access logging enabled to provide an audit trail and visibility into data access patterns. The invariant flags any bucket where `logging.enabled` is `false`.
...
An observation with logging enabled satisfies this invariant:
...
- [`INV.S3.GOVERNANCE.001`](../lifecycle-governance/inv-s3-governance-001) -- Data Classification Tag Required. Buckets without a `data-classification` tag silently bypass sensitivity-gated checks. Logging and classification together ensure that access to sensitive buckets is both tracked and categorizable.
