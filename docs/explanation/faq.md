...
## Why does Stave use "unsafe state" instead of "vulnerability" or "misconfiguration"?[​](#why-does-stave-use-unsafe-state-instead-of-vulnerability-or-misconfiguration "Direct link to Why does Stave use \"unsafe state\" instead of \"vulnerability\" or \"misconfiguration\"?")
...
## What is "System Invariant as Code"?[​](#what-is-system-invariant-as-code "Direct link to What is \"System Invariant as Code\"?")
...
## Why "control" and not "rule" or "policy"?[​](#why-control-and-not-rule-or-policy "Direct link to Why \"control\" and not \"rule\" or \"policy\"?")
...
## Why is there a semantic gap between the domain and the code?[​](#why-is-there-a-semantic-gap-between-the-domain-and-the-code "Direct link to Why is there a semantic gap between the domain and the code?")
...
## Why does Stave need two snapshots?[​](#why-does-stave-need-two-snapshots "Direct link to Why does Stave need two snapshots?")
...
## Why does Stave work offline with no credentials?[​](#why-does-stave-work-offline-with-no-credentials "Direct link to Why does Stave work offline with no credentials?")
...
## What S3 blind spots does Stave detect that AWS Trusted Advisor misses?[​](#what-s3-blind-spots-does-stave-detect-that-aws-trusted-advisor-misses "Direct link to What S3 blind spots does Stave detect that AWS Trusted Advisor misses?")
...
### 1. Policy-denied scanning (the Fog Security bypass)[​](#1-policy-denied-scanning-the-fog-security-bypass "Direct link to 1. Policy-denied scanning (the Fog Security bypass)")
...
### 2. Latent public exposure behind Public Access Block[​](#2-latent-public-exposure-behind-public-access-block "Direct link to 2. Latent public exposure behind Public Access Block")
...
### 3. ACL escalation paths[​](#3-acl-escalation-paths "Direct link to 3. ACL escalation paths")
...
### Detection comparison[​](#detection-comparison "Direct link to Detection comparison")
...
| ACL escalation (WRITE\_ACP)        | Not checked                          | `CTL.S3.ACL.ESCALATION.001` — flags privilege escalation path       |
...
## How does Stave keep developer-only commands out of production?[​](#how-does-stave-keep-developer-only-commands-out-of-production "Direct link to How does Stave keep developer-only commands out of production?")

Stave has **no destructive commands** — every command reads observation snapshots and writes findings; nothing modifies or deletes infrastructure. There is no "accidental destruction" for Stave to cause.

A few commands are **developer-workflow** tools, though, that have no place in a production environment:

* `generate` — scaffolds starter artifacts (project/observation templates).
* `forge` — authors and tests custom controls.

These are tagged **dev-only** in the CLI (a Cobra annotation, `cmdutil.AnnotationDevOnly`, on the command). Before any command runs, the bootstrap guard checks two things:

1. **Is the command dev-only?** The check walks the command's ancestry, so tagging `forge` or `generate` automatically covers every subcommand (`forge new`, `generate observation`, …).

2. **Is this a production environment?** Detected via `STAVE_ENV=production`, or an active context with `production: true`:
...
If both are true, the command is **rejected** with a clear error and a non-zero exit code:
...
The dev-only set is declared **in code**, not via config — there is no `blocked_commands` list to set or keep in sync. Marking a new command dev-only is one annotation: `cmdutil.AnnotationDevOnly: "true"`.

## Why are there two binaries (`stave` and `stave-dev`)?[​](#why-are-there-two-binaries-stave-and-stave-dev "Direct link to why-are-there-two-binaries-stave-and-stave-dev")
...
The edition is a **build-time diagnostic label** — it fixes what `--version` and crash hints report, so there is no runtime `--dev` flag to accidentally set in a CI script. It is *not* a safety toggle: the development-only command guard (see above) is keyed on the production environment, not on the edition, so **both** binaries reject `forge` / `generate` in production.
...
## Why is there an OSCAL control in core *and* an `oscal-gaps` command in stave-coverage?[​](#why-is-there-an-oscal-control-in-core-and-an-oscal-gaps-command-in-stave-coverage "Direct link to why-is-there-an-oscal-control-in-core-and-an-oscal-gaps-command-in-stave-coverage")

They answer two different questions about OSCAL, so they live in two different places. The shared word "OSCAL" is just the input format.

`CTL.COMPLIANCE.OSCAL.REPORT.CURRENT.001` (core catalog) is a runtime invariant over a **customer's snapshot**. It asks: *"Is the SOC report this account relies on present, unexpired (≤ 12 months past its coverage period), and in scope for the snapshot's region?"* It evaluates a derived `oscal_report` asset during `stave apply` and emits a finding — exactly what a control is, so it lives in the catalog with every other control.

`stave-coverage oscal-gaps` never looks at a customer snapshot. It asks a **meta question about the catalog itself**: *"Of the customer- and shared-responsibility SOC 2 criteria AWS lists in its OSCAL report, which ones does the Stave catalog have no control for?"* That is catalog-coverage analysis, which is the entire reason `stave-coverage` exists as a separate tool — coverage logic never lives in core.
...
No shared code (separate Go modules), no overlapping logic. The control is the shippable customer-facing invariant; `oscal-gaps` is an internal catalog-completeness tool that stands beside it.

## What is the output contract schema (`out.v0.1`)?[​](#what-is-the-output-contract-schema-outv01 "Direct link to what-is-the-output-contract-schema-outv01")
...
### Evaluation output structure[​](#evaluation-output-structure "Direct link to Evaluation output structure")
...
### Design decisions[​](#design-decisions "Direct link to Design decisions")
...
### Accessing the output[​](#accessing-the-output "Direct link to Accessing the output")
...
Full field-by-field reference: [Output Schema (out.v0.1)](/docs/reference/schema-out.md)

JSON Schema source: `schemas/output/v1/output.schema.json`

## What does Stave *not* do?[​](#what-does-stave-not-do "Direct link to what-does-stave-not-do")
...
## Are all controls YAML or are some implemented in Go?[​](#are-all-controls-yaml-or-are-some-implemented-in-go "Direct link to Are all controls YAML or are some implemented in Go?")
...
## Google CEL Library[​](#google-cel-library "Direct link to Google CEL Library")

For the predicate engine that is mostly using google cel library, is it possible to open a PR that moves that functionality to it? Is it appropriate to ask the library maintainer to implement it (is it in their scope and generic enough?)<br />No, it's not appropriate. Stave's CEL usage is domain-specific — it evaluates unsafe\_predicate YAML structures (with all/any/field/op/value nodes) against asset property maps. This is a DSL built on top of CEL, not CEL itself.

What stave does:

1. Parses ctrl.v1 YAML predicates into an internal UnsafePredicate tree
2. Translates field ops (eq, ne, in, missing, present, not\_subset\_of\_field) into CEL expressions
...
* The unsafe\_predicate YAML schema is stave's invention, not a CEL concept
* The field operator set (eq, ne, missing, present, not\_subset\_of\_field) is stave's domain language
...
Option C: Mounted volume in Docker The extractor writes to a directory, the stave container mounts it: docker run --rm -v $(pwd)/snapshots\:/work/observations stave-tutorials<br />stave apply --observations observations --max-unsafe 7d --format json
...
The CI workflow in stave-guide/how-to/integration/ci-cd-integration.md documents all the patterns: GitHub Actions (build from source or Docker), GitLab CI, baseline tracking, SARIF upload, and gating.

## What is the purpose of now flag in apply command?[​](#what-is-the-purpose-of-now-flag-in-apply-command "Direct link to What is the purpose of now flag in apply command?")
...
## Which ones are validated using JSON schema?[​](#which-ones-are-validated-using-json-schema "Direct link to Which ones are validated using JSON schema?")
...
Command Output Contracts: baseline baseline.v0.1 ci\_diff ci\_diff.v0.1 enforce enforce.v0.1 fix\_loop fix\_loop.v0.1 gate gate.v0.1 snapshot\_archive snapshot\_archive.v0.1 snapshot\_plan snapshot\_plan.v0.1 snapshot\_quality snapshot\_quality.v0.1 validate validate.v0.1

Artifact Contracts: bug\_report bug-report.v0.1 control\_crosswalk\_resolution control-crosswalk-resolution.v1 security\_audit security-audit.v1 security\_audit\_artifacts security-audit-artifacts.v1 security\_audit\_run\_manifest security-audit-run-manifest.v1

Validated with JSON Schema (4):

┌─────────────┬─────────────────────────────────────────────────────────────────┐<br />│ Schema │ Validation Function │<br />├─────────────┼─────────────────────────────────────────────────────────────────┤<br />│ ctrl.v1 │ ValidateControlYAML() — control loader │<br />├─────────────┼─────────────────────────────────────────────────────────────────┤<br />│ obs.v0.1 │ ValidateObservationJSON() — observation loader │<br />├─────────────┼─────────────────────────────────────────────────────────────────┤<br />│ out.v0.1 │ ValidateEvaluation() / ValidateVerification() — safety envelope │<br />├─────────────┼─────────────────────────────────────────────────────────────────┤<br />│ diagnose.v1 │ ValidateDiagnose() — safety envelope │ └─────────────┴─────────────────────────────────────────────────────────────────┘

Plus finding.v1 (internal, not in stave schemas output) — validated optionally in the finding writer.

No JSON Schema (20):

All command output contracts (baseline.v0.1, ci\_diff.v0.1, enforce.v0.1, gate.v0.1, validate.v0.1, etc.), all artifact contracts (security-audit.v1, bug-report.v0.1, etc.), and diff.v0.1 exist only as version constants in kernel/schema.go. They're stamped into output JSON as schema\_version fields but have no .schema.json file and no runtime validation.
...
## How are the testscripts structured?[​](#how-are-the-testscripts-structured "Direct link to How are the testscripts structured?")
...
## What is the Logic Trace (`apply --trace`)?[​](#what-is-the-logic-trace-apply---trace "Direct link to what-is-the-logic-trace-apply---trace")

`stave apply --trace <file>` records the evaluation engine's reasoning chain — the step-by-step decisions (exemption checks, predicate evaluations, threshold checks, and the final verdict) for every control × asset pair. It answers: *"Why did the engine reach this verdict?"*

Every finding already carries a compact `reasoning_trace` inline (rendered as prose in text output, as raw DSL in JSON/SARIF). The `--trace` flag writes the full `Assessment.Steps[]` superset to a separate file for users who want the precise predicate-DSL form or per-step timing.
...
For LLM-driven remediation you can also consume `eval.json` directly: controls carry triage terms (defect, infection, failure) so the findings are already self-explanatory.
