...
## AirGap Policy Source of Truth[​](#airgap-policy-source-of-truth "Direct link to AirGap Policy Source of Truth")
...
## No subprocess execution[​](#no-subprocess-execution "Direct link to No subprocess execution")
...
## No plugins[​](#no-plugins "Direct link to No plugins")
...
## Restricted template execution[​](#restricted-template-execution "Direct link to Restricted template execution")
...
## No interpreters or embedded runtimes[​](#no-interpreters-or-embedded-runtimes "Direct link to No interpreters or embedded runtimes")

Stave does not embed scripting engines, Lua, WASM runtimes, or any other code execution environment.

## No reflection-based dispatch[​](#no-reflection-based-dispatch "Direct link to No reflection-based dispatch")
...
## Controls are a closed DSL[​](#controls-are-a-closed-dsl "Direct link to Controls are a closed DSL")

Control files are data (YAML), not code. They are:

* Schema-validated against `ctrl.v1` before evaluation
* Restricted to a fixed set of predicate operators (see [Evaluation Semantics](/docs/reference/evaluation-semantics.md#predicate-operator-reference))
...
## Enforcement artifacts are templates, not executables[​](#enforcement-artifacts-are-templates-not-executables "Direct link to Enforcement artifacts are templates, not executables")
...
## Banned imports summary[​](#banned-imports-summary "Direct link to Banned imports summary")
