...
* **Verdict produced:** NON\_COMPLIANT. The metadata endpoint accepts unauthenticated reads while a forgeable surface is exposed; the instance role's credentials are reachable.
...
| Argue over severity of two separate low findings                                             | A single NON\_COMPLIANT verdict naming the actual exposure                                                |
