...
This tutorial walks through running chain discovery on a snapshot to find every privilege escalation and data exfiltration path.
...
The snapshot must contain raw IAM policy data — specifically `trust_policy_json` and `policies_json` fields on IAM role assets. Without these, the SIR projectors cannot emit the access-graph edges that the Datalog engine traverses.

## Step 1: Export SIR Facts[​](#step-1-export-sir-facts "Direct link to Step 1: Export SIR Facts")

The chain discovery engine operates on Structured Intermediate Representation (SIR) facts — relationship triples extracted from your snapshot by 21 projectors in `internal/core/sirfacts/facts.go`.
...
## Step 2: Run Chain Discovery[​](#step-2-run-chain-discovery "Direct link to Step 2: Run Chain Discovery")
...
## Step 3: Read the Report[​](#step-3-read-the-report "Direct link to Step 3: Read the Report")
...
### Novel Chains[​](#novel-chains "Direct link to Novel Chains")

These are the discoveries — paths that exist in your account but aren't covered by any of the 622 known chain patterns:
...
### Confirmed Chains[​](#confirmed-chains "Direct link to Confirmed Chains")
...
## Step 4: Focus on Specific Query Types[​](#step-4-focus-on-specific-query-types "Direct link to Step 4: Focus on Specific Query Types")
...
## Step 5: JSON Output for Integration[​](#step-5-json-output-for-integration "Direct link to Step 5: JSON Output for Integration")
...
## Common Patterns Chain Discovery Finds[​](#common-patterns-chain-discovery-finds "Direct link to Common Patterns Chain Discovery Finds")
...
## How Chain Discovery Differs from Per-Resource Controls[​](#how-chain-discovery-differs-from-per-resource-controls "Direct link to How Chain Discovery Differs from Per-Resource Controls")

Per-resource controls check individual configurations: "Is this S3 bucket encrypted?" "Does this role have wildcard permissions?" "Is this security group too broad?"

Chain discovery checks relationships between resources: "Can a Cognito user reach production S3 data through a three-hop trust chain involving a Lambda function and a SageMaker role?"

Both are necessary. Per-resource controls catch individual misconfigurations. Chain discovery catches compound risks where every individual configuration looks acceptable but the combination creates an attack path.
...
All three resources pass individual checks. The chain between them fails. This is the gap that chain discovery closes.

## Troubleshooting[​](#troubleshooting "Direct link to Troubleshooting")

**All zeros in the report:**

The observation snapshots are missing the raw IAM policy fields. Check which SIR predicates are being emitted:
...
Look for `can_assume`, `has_action`, `has_resource`, `trusts_service`. If missing, the observation JSON needs:
...
See `reasoning/souffle/discovery/LIVE-VALIDATION.md` for a complete runbook to create test data from a live AWS account.
