...
sidebar_label: "BCM #764243"
sidebar_position: 20
description: "A presigned POST policy uses an empty starts-with prefix, so any key in the bucket is a legal upload target — can you prove the write scope is unrestricted from the policy alone?"
---
...
To answer by hand you have to read the condition as S3 evaluates it: `starts-with
$key, ""` matches *every* string, because every string starts with the empty
