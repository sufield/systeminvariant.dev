...
sidebar_label: "CTL.S3.PUBLIC.005"
sidebar_position: 5
...
- [`CTL.S3.PUBLIC.006`](inv-s3-public-006.md) -- The listing counterpart: detects latent public bucket listing masked only by PAB.
- [`CTL.S3.PUBLIC.001`](inv-s3-public-001.md) -- Detects active (not latent) public read access where PAB is not blocking it.
- [`CTL.S3.CONTROLS.001`](inv-s3-controls-001.md) -- Ensures PAB is enabled; this control catches the case where PAB is enabled but is the only thing standing between the bucket and the internet.
