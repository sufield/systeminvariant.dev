...
- [`CTL.S3.LOCK.002`](inv-s3-lock-002.md) -- Requires COMPLIANCE mode on PHI Object Lock; this control checks the retention duration is long enough.
- [`CTL.S3.LOCK.001`](inv-s3-lock-001.md) -- Requires Object Lock to be enabled on compliance-tagged buckets.
- [`CTL.S3.LIFECYCLE.002`](inv-s3-lifecycle-002.md) -- Enforces the same 2190-day HIPAA minimum on lifecycle expiration rules; both controls must pass for full coverage.
