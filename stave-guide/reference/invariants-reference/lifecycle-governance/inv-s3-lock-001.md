...
- [`CTL.S3.LOCK.002`](inv-s3-lock-002.md) -- Requires COMPLIANCE mode (not GOVERNANCE) for PHI buckets with Object Lock.
- [`CTL.S3.LOCK.003`](inv-s3-lock-003.md) -- Enforces minimum retention period on Object Lock for PHI buckets.
- [`CTL.S3.VERSION.001`](inv-s3-version-001.md) -- Versioning is a prerequisite for Object Lock.
- [`CTL.S3.GOVERNANCE.001`](inv-s3-governance-001.md) -- The `compliance` tag that gates this control works alongside the `data-classification` tag.
