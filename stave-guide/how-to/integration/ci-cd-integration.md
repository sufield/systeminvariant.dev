...
sidebar_position: 1
description: "How to integrate Stave into GitHub Actions, GitLab CI, and Jenkins pipelines."
---
...
Exit code 3 (violations found) fails the pipeline stage. To gate
merges, run the evaluation stage on PRs and let Jenkins block the
merge on failure.
...
Stave produces SARIF v2.1.0 output that GitHub Actions automatically
renders in the "Security" tab and highlights violations in PR diffs:
...
This surfaces HIPAA violations directly in the "Files Changed" tab of
pull requests — no extra tooling needed.
...
Compare the current evaluation against a baseline to show exactly which
violations are new and which were resolved:
...
Resolution priority: environment variable > project config > user
config > default. This allows per-environment overrides without
changing the command invocation.
