...
1. **Extract** — use an extractor (any language) to produce `obs.v0.1` JSON from your AWS snapshot directory. The input directory should contain files like `list-buckets.json`, `get-bucket-acl/<bucket>.json`, etc. See [Building an Extractor](../integration/building-extractors.md) for a jumpstart template.
...
1. **Capture before-state observations** (pre-remediation) — use an extractor to produce `obs.v0.1` JSON from your AWS snapshot. See [Building an Extractor](../integration/building-extractors.md).
